...
function pollPservs(ns, args) {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    if (ns.serverExists(name)) {
      let sv = ns.getServer(name);
      ns.tprint(`${name}: ${ns.formatRam(sv.ramUsed)}/${ns.formatRam(sv.maxRam)}`);
    }
  }
}
...
function printHelp(ns, args) {
  ...
  ns.tprint("  poll      - display a list of all player servers, as well as their used/total ram.");
  ns.tprint("  rn        - rename an existing server.");
  ns.tprint("Usage:");
  ns.tprint("  run sv-util.js <flag> <arg1> <arg2> ... etc");
}
